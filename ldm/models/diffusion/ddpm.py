...
class DDPM(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        aaa, loss_dict_no_ema = self.shared_step(batch)
        with self.ema_scope():
...
class MyDDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 monitor="val/loss",
                 use_ema=True,
                 first_stage_key_1="image_1",
                 first_stage_key_2="image_2",
                 image_size=256,
                 channels=3,
                 log_every_t=100,
                 clip_denoised=True,
        ...
        self.clip_denoised = clip_denoised
        self.log_every_t = log_every_t
        self.first_stage_key_1 = first_stage_key_1
        self.first_stage_key_2 = first_stage_key_2
        self.image_size = image_size  # try conv?
        self.channels = channels
        self.use_positional_encodings = use_positional_encodings
        self.model = DiffusionWrapper(unet_config, conditioning_key)
    ...
    def shared_step(self, batch):
        x = self.get_input(batch, self.first_stage_key_1)
        loss, loss_dict = self(x)
        return loss, loss_dict
...
class MyLatentDiffusion(MyDDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
                 personalization_config,
                 num_timesteps_cond=None,
                 cond_stage_key_1="image_1",
                 cond_stage_key_2="image_2",
                 cond_stage_trainable=False,
                 concat_mode=True,
                 cond_stage_forward=None,
                 conditioning_key=None,
        ...
        self.concat_mode = concat_mode
        self.cond_stage_trainable = cond_stage_trainable
        self.cond_stage_key_1 = cond_stage_key_1
        self.cond_stage_key_2 = cond_stage_key_2
        try:
            self.num_downs = len(first_stage_config.params.ddconfig.ch_mult) - 1
        except:
            self.num_downs = 0
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            # set rescale weight to 1./std of encodings
            print("### USING STD-RESCALING ###")
            x = super().get_input(batch, self.first_stage_key_1)
            x = x.to(self.device)
            encoder_posterior = self.encode_first_stage(x)
            z = self.get_first_stage_encoding(encoder_posterior).detach()
            del self.scale_factor
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            if cond_key is None:
                cond_key = self.cond_stage_key_1
            if cond_key != self.first_stage_key_1:
                if cond_key in ['caption_1', 'caption_2','coordinates_bbox']:
                    xc = batch[cond_key]
                elif cond_key == 'class_label':
                    xc = batch
                else:
    ...
    def shared_step(self, batch, **kwargs):
        x_1, c_1 = self.get_input(batch, self.first_stage_key_1)
        x_2, c_2 = self.get_input(batch, self.first_stage_key_2)
        x=torch.concat((x_1,x_2),0)
        c=c_1.append(c_2)
        print("          type:x c")
        ...
        print(c)
        loss = self(x, c)
        '''x, c = self.get_input(batch, self.first_stage_key_2)
        loss_2 = self(x, c)
        loss = loss_1 + loss_2'''
        return loss
